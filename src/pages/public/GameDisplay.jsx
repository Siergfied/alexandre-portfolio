...
export default function GameDisplay() {
	...
	return (
		<div className='h-full w-full relative overflow-hidden border-8 border-[#8759B4] rounded-sm px-16 py-12 bg-zinc-700'>
						...
						<ul className='flex flex-wrap gap-4'>
							{categoryData.map(({ id, title, icon }) =>
								gameData[0].category.includes(id) ? (
									<li key={id} className='flex items-center justify-center gap-2 py-2 h-10 w-40 bg-zinc-800 rounded-md border-2 border-[#8759B4]'>
