...
export default function ImageFormBis({ id, title, description, formHandler, disabled, coverImageCropped, setCoverImageCropped, backgroundImageCropped, setBackgroundImageCropped, children }) {
	...
	return (
		<>
			<div className='bg-white shadow sm:rounded-lg sm:p-6'>
				<form onSubmit={formHandler} className='flex flex-col gap-6' id={id}>
					<div className='flex justify-between'>
						<div>
							...
							<div className={'relative flex items-center justify-center rounded-md mt-1 border border-gray-300 ' + (disabled ? 'bg-zinc-200' : '')}>
								<div className='z-0 h-96 w-72 m-2 '>{coverImageCropped && <img src={handleCoverImageCropped()} className='w-full' />}</div>
								{!disabled && (
									<label
										htmlFor={id + '_cover'}
						...
						<div>
							...
							<div className={'relative flex items-center justify-center rounded-md mt-1 border border-gray-300 ' + (disabled ? 'bg-zinc-200' : '')}>
								<div className='z-0 h-96 w-[48rem] m-2'>{backgroundImageCropped && <img src={handleBackgroundImageCropped()} className='w-full' />}</div>
								{!disabled && (
									<label
					...
					<div className='flex flex-col grow'>
						...
						<div className='mt-1 flex flex-col h-full'>
							...
							<textarea
								name='description'
								id={id + '_description'}
								className={'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full h-60 resize-none ' + (disabled ? 'bg-zinc-200' : '')}
					...
					<div className='flex w-full justify-end w-32'> {children}</div>
				</form>
			</div>
