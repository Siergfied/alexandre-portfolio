...
export default function GameForm({ id, url, setUrl, title, setTitle, description, setDescription, category, categoryList, formAction, disabled, children }) {
	...
	return (
		<form onSubmit={handleForm} className='flex flex-col bg-zinc-100 shadow sm:rounded-lg sm:p-6 text-zinc-800' id={id}>
				...
				<div className='flex flex-col w-full gap-2'>
					...
					<div className='flex flex-col h-full'>
						...
						<textarea
							name='description'
							id={id + '_description'}
							className={'border-zinc-400 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full h-40 resize-none ' + (disabled ? 'bg-zinc-400' : 'bg-zinc-100')}
							value={description}
							disabled={disabled}
